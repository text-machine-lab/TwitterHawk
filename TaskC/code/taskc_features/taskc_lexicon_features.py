#-------------------------------------------------------------------------------
# Name:        taskc_lexicon_features.py
#
...
from utilities import normalize_phrase
...
def light_normalize(sentence, begin, end, ark_tweet):
    ...
    # Normalize phrase
    normalized = normalize_phrase(sentence, ark_tweet)

    # Get given phrase (note. assumes that 'end' accounts for off-by-one)
    phrase = normalized[begin:end]

    return [  w.lower() for words in phrase for w in words ]
