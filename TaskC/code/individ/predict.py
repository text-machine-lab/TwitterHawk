...
def main():
    ...
    for txt_file in txt_files:
        note = Note()
        note.read(txt_file)
        data = zip(note.getTweets(), note.getTopics())

        labels = []

        # Break into multiple topics (one classifier for each)
        groups = defaultdict(lambda:[])
        for twt,topic in data:
            groups[topic].append(twt)

        # Classify for each topic
        outfile  = os.path.join(out_dir, os.path.basename(txt_file))
        for topic,X in sorted(groups.items(),key=lambda t:t[1]):
            ...
            if os.path.exists(model+'.model'):
                labels += predict_using_model(X, model)
            else:
                # Most common label from training data
                labels += [ 'neutral' for _ in X ]
